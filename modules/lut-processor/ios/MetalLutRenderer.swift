...
final class MetalLutRenderer {
  ...
  private func applyHaldLut(
    ...
    let lutTexture = try loadTexture(path: haldPngPath, srgb: false)
    ...
    guard let outTexture = makeOutputTexture(width: width, height: height) else {
      throw MetalLutError.renderFailed
    }
    ...
    var uni = LutUniforms(level: level, intensity: intensity, padding: .zero)
    try renderToTexture(
    ...
    return try writeTextureToJpeg(outTexture, width: width, height: height, quality: quality).path
  }
  ...
  private func applyCubeLut(
    ...
    let cached = try cachedCubeTexture(for: cubePath)
